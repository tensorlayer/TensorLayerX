...
def model(x, is_train, reuse):
    with tf.variable_scope("STN", reuse=reuse):
        nin = tensorlayerx.layers.InputLayer(x, name='in')
        ## 1. Localisation network
        # use MLP as the localisation net
        nt = tensorlayerx.layers.FlattenLayer(nin, name='flatten')
        nt = tensorlayerx.layers.DenseLayer(nt, n_units=20, act=tf.nn.tanh, name='dense1')
        nt = tensorlayerx.layers.DropoutLayer(nt, keep=0.8, is_fix=True, is_train=is_train, name='drop1')
        ...
        n = tensorlayerx.layers.SpatialTransformer2dAffineLayer(nin, theta_layer=nt, out_size=(40, 40), name='spatial')
        s = n
        ## 3. Classifier
        ...
        n = tensorlayerx.layers.FlattenLayer(n, name='flatten2')
        n = tensorlayerx.layers.DenseLayer(n, n_units=1024, act=tf.nn.relu, name='out1')
        n = tensorlayerx.layers.DenseLayer(n, n_units=10, name='out2')
    return n, s
