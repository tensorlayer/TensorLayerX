...
def model(x, is_train=True, reuse=False):
    with tf.variable_scope("model", reuse=reuse):
        ...
        n = tensorlayerx.layers.Conv2d(n, n_filter=80, name='conv2d_1')
        n = tensorlayerx.layers.BatchNormLayer(n, is_train=is_train, name='norm_batch')
        n = tensorlayerx.layers.Conv2d(n, n_filter=80, name='conv2d_2')
        n = tensorlayerx.layers.LocalResponseNormLayer(n, name='norm_local')
        n = tensorlayerx.layers.LayerNormLayer(n, reuse=reuse, name='norm_layer')
        n = tensorlayerx.layers.InstanceNormLayer(n, name='norm_instance')
        # n = tl.layers.GroupNormLayer(n, groups=40, name='groupnorm')
        n.outputs = tf.reshape(n.outputs, [-1, 80, 100, 100])
        n = tensorlayerx.layers.GroupNormLayer(n, groups=40, data_format='channels_first', name='groupnorm')
        n.outputs = tf.reshape(n.outputs, [-1, 100, 100, 80])
        n = tensorlayerx.layers.SwitchNormLayer(n, name='switchnorm')
        n = tensorlayerx.layers.QuanConv2dWithBN(n, n_filter=3, is_train=is_train, name='quan_cnn_with_bn')
        n = tensorlayerx.layers.FlattenLayer(n, name='flatten')
        n = tensorlayerx.layers.QuanDenseLayerWithBN(n, n_units=10, name='quan_dense_with_bn')
    return n
