...
def activation_module(layer, activation_fn, leaky_relu_alpha=0.2, name=None):
    ...
    if activation_fn not in ["ReLU", "ReLU6", "Leaky_ReLU", "PReLU", "PReLU6", "PTReLU6", "CReLU", "ELU", "SELU",
        ...
        layer = tensorlayerx.layers.LambdaLayer(prev_layer=layer, fn=tf.nn.relu, name=act_name)

    elif activation_fn == "ReLU6":
        layer = tensorlayerx.layers.LambdaLayer(prev_layer=layer, fn=tf.nn.relu6, name=act_name)

    elif activation_fn == "Leaky_ReLU":
        layer = tensorlayerx.layers.LambdaLayer(
            prev_layer=layer, fn=tf.nn.leaky_relu, fn_args={'alpha': leaky_relu_alpha}, name=act_name
        )

    elif activation_fn == "PReLU":
        layer = tensorlayerx.layers.PReluLayer(prev_layer=layer, channel_shared=False, name=act_name)

    elif activation_fn == "PReLU6":
        layer = tensorlayerx.layers.PRelu6Layer(prev_layer=layer, channel_shared=False, name=act_name)

    elif activation_fn == "PTReLU6":
        layer = tensorlayerx.layers.PTRelu6Layer(prev_layer=layer, channel_shared=False, name=act_name)

    elif activation_fn == "CReLU":
        layer = tensorlayerx.layers.LambdaLayer(prev_layer=layer, fn=tf.nn.crelu, name=act_name)

    elif activation_fn == "ELU":
        layer = tensorlayerx.layers.LambdaLayer(prev_layer=layer, fn=tf.nn.elu, name=act_name)

    elif activation_fn == "SELU":
        layer = tensorlayerx.layers.LambdaLayer(prev_layer=layer, fn=tf.nn.selu, name=act_name)

    elif activation_fn == "tanh":
        layer = tensorlayerx.layers.LambdaLayer(prev_layer=layer, fn=tf.nn.tanh, name=act_name)

    elif activation_fn == "sigmoid":
        layer = tensorlayerx.layers.LambdaLayer(prev_layer=layer, fn=tf.nn.sigmoid, name=act_name)

    elif activation_fn == "softmax":
        layer = tensorlayerx.layers.LambdaLayer(prev_layer=layer, fn=tf.nn.softmax, name=act_name)
...
def conv_module(
    ...
    batch_norm_init=tensorlayerx.nn.initializers.truncated_normal(mean=1., stddev=0.02),
    ...
    layer = tensorlayerx.layers.Conv2d(
        prev_layer,
    ...
    if use_batchnorm:
        ...
        layer = tensorlayerx.layers.BatchNormLayer(
            layer, act=None, is_train=is_train, gamma_init=batch_norm_init, name=bn_name
        )
...
def dense_module(
    ...
    if prev_layer.outputs.get_shape().__len__() != 2:  # The input dimension must be rank 2
        layer = tensorlayerx.layers.FlattenLayer(prev_layer, name='flatten')

    else:
        layer = prev_layer

    layer = tensorlayerx.layers.DenseLayer(
        layer,
        n_units=n_units,
        act=None,
        W_init=dense_init,
