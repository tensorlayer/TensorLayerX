...
def test(network, acc, X_test, y_test, batch_size, cost=None):
    """
    Test a given non time-series network by the given test data and metrics.
...
def train_epoch(
    ...
    """Training a given non time-series network by the given losses function, training data, batch_size etc.
    for one epoch.
    ...
    acc_ep : Tensor or None. Average accuracy(metrics) of this epoch. None if acc is not given.
...
def run_epoch(network, X, y, cost=None, acc=None, batch_size=100, shuffle=False):
    """Run a given non time-series network by the given losses function, test data, batch_size etc.
    for one epoch.
    ...
    Returns
    -------
    loss_ep : Tensor. Average loss of this epoch. None if 'losses' is not given.
    acc_ep : Tensor. Average accuracy(metrics) of this epoch. None if 'acc' is not given.
    n_step : int. Number of iterations taken in this epoch.
    """
    network.eval()
    loss_ep = 0
